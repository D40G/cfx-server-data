...
exports('discord', function(message, id, id2, color, channel)
	...
	if message == nil then print("^1Error: DX-Logs Export. Invalid message.^0") return end
	if id == nil or id == "PLAYER_ID" or not tonumber(id) then print("^1Error: DX-Logs Export. Invalid player id.^0") return end
	if id == nil or id2 == "PLAYER_2_ID" or not tonumber(id2) then print("^1Error: DX-Logs Export. Invalid second player id.^0") return end
	if color == nil then print("^1Error: DX-Logs Export. Invalid color.^0") return end
	if channel == nil or channel == "" then print("^1Error: DX-Logs Export. Invalid channel.^0") return end
...
Citizen.CreateThread( function()
		SetConvarServerInfo("DX-Logs", "V"..Config.versionCheck)
		local vRaw = LoadResourceFile(GetCurrentResourceName(), 'version.json')
		if vRaw and Config.versionCheck then
			local v = json.decode(vRaw)
			PerformHttpRequest(
				'https://raw.githubusercontent.com/Prefech/DX-Logs/master/version.json',
...
^0]]):format(
						...
						print('DX-Logs unable to check version')
					end
				end,
				'GET'
			)
